...
(function ()
{
	...
	{
		return cr.equals_nocase(file_, this.justLoaded);
	};
	...
	{
		this.setReturn( window[func_].apply(window,params_) );
	};
	
	Acts.prototype.FunctionWithObjects = function (func_, obj_, params_)
	{
		params_.unshift(obj_.getCurrentSol().getObjects());
		this.setReturn( window[func_].apply(window,params_) );
	};
	...
	Exps.prototype.Function = function (ret, name_)
	{
		...
		ret.set_any( this.setReturn(window[name_].apply( window,arguments )) );
	};
